server <- function(session, input, output) {
  ...
  filtered_data <- eventReactive(input$render_plot, {
    unemp %>%
      filter(place_name %in% input$place_name_selected & level %in% input$level_selected)
  })
  ...
  #
  legend_selected <- eventReactive(input$render_plot, {
  ...
  output$unemp_plot <- renderPlot({
    ...
    ggplot(data = filtered_data()) +
      ...
      geom_line(aes(year, !!rlang::sym(variable_selected()), color = paste(place_name, level, sep = "-"))) +
      #geom_line(aes_string("year", variable_selected(), color = "place_name")) +
      labs(title = plot_title()) +
      theme(legend.position = legend_selected())
  ...
  output$unemp_plot_g2r <- renderG2({
    ...
    g2(data = filtered_data()) %>%
      ...
      fig_line(asp(year, !!rlang::sym(variable_selected()), color = place_name, size = level)) %>%
      # For some reason, the title does not show...
      subject(plot_title()) %>%
      legend_color(position = legend_selected())
